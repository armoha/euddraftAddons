...
from eudplib.eudlib.memiof.modcurpl import f_getcurpl, f_setcurpl
...
def f_dwepdcunitread_epd(targetplayer):
    ...
    c.RawTrigger(actions=[ptr.SetNumber(0), epd.SetNumber(0)])
    fin << c.RawTrigger(
        actions=[addact << c.SetDeaths(c.CurrentPlayer, c.Add, 0xEDAC, 0)]
    )
    f_setcurpl(origcp)
