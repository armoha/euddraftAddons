...
- QCSafety: Respawn QCUnit when something bad happened.
"""
...
try:
    ...
    from eudx import MemoryX, MemoryXEPD, DeathsX, SetDeathsX, SetMemoryX, SetMemoryXEPD
try:
...
try:
    ...
    import eudx
    f_mapXread_epd = lambda epd: eudx.f_dwread_epd(epd, mask=2**(mapX + 1) - 1)
    f_mapYread_epd = lambda epd: eudx.f_dwread_epd(epd, mask=2**(mapY + 1) - 1)
    f_screenXread_epd = lambda epd: eudx.f_dwread_epd(epd, mask=0x3FF)
    f_screenYread_epd = lambda epd: eudx.f_dwread_epd(epd, mask=0x1FF)
    f_epdcunitread_epd = lambda epd: eudx.f_epdread_epd(epd, mask=0x7FFFF8)
    f_cunitread_cp = lambda cpoffset: eudx.f_dwread_cp(cpoffset, mask=0x7FFFF8)
else:
    fxx = lambda x: x
    f_mapXread_epd = f_readgen_epd(2**(mapX + 1) - 1, (0, fxx))
    f_mapYread_epd = f_readgen_epd(2**(mapY + 1) - 1, (0, fxx))
    f_screenXread_epd = f_readgen_epd(0x3FF, (0, fxx))
    f_screenYread_epd = f_readgen_epd(0x1FF, (0, fxx))
...
_once = EUDVariable()
pv5 = EUDVariable()
MapXY = [2**i for i in range(mapX + 1)] + [2**j for j in range(mapY + 1)]
XYPool = cycle(MapXY)
_counter = 0
...
def _AddLoc(loc, _fdict={}):
    if loc in _fdict:
        ...
        def _f(_firstcall=[]):
            ...
            if _counter > mapX:
                lc += 1
            DoActions([  # pv5 = 5 * f_getcurpl() + EPD(0x58DC60)
...
def Respawn():
    ...
    for i in EUDLoopRange(QCCount):
        ...
        DoActions([
            ...
            SetMemoryEPD(epd + 0x4C // 4, Subtract, QCPlayer - f_getcurpl()),  # modify unit's player
        ...
        for pi, p in enumerate(human):
            for j in range(QCCount):
                RawTrigger(
                    conditions=[
                        Memory(0x6509B0, Exactly, p),
                        i.Exactly(j),
                    ],
                    actions=SetMemory(getepd + 20, SetTo, EPD(EPDList[pi][j].getValueAddr()))
                )
        VProc(epd, [
        ...
        if EUDIf()(Memory(0x57F1B0, Exactly, f_getcurpl())):
            f_dwwrite_epd(SetAlphaArray[i], epd2alphaid(epd))
            getptr = Forward()
            for j in range(QCCount):
                RawTrigger(
...
def Start():
    ...
    DoActions([
        SetMemory(act + 20, SetTo, localcp)
        for act in setlocalcp
    ])
    Respawn()
...
@EUDFunc
def QueueGameCommand_NSelect(n, ptrListepd):
...
def Main():
        ...
        for i in EUDLoopRange(QCCount):
            ...
            for pi, p in enumerate(human):
                for j, epd in enumerate(EPDList[pi]):
                    ...
                    _next << NextTrigger()
            _skip << RawTrigger(
                actions=[_restore_nextptr << SetNextPtr(0, 0)])
            if EUDIfNot()(DeathsX(CurrentPlayer, Exactly, cp, 0, 0xFF)):
                RawTrigger(
                    nextptr=something_bad_happened,
                    actions=QCBroken.SetNumber(1)
                )
    ...
    for n, l in enumerate(chunks(QCList, mapXY)):
        DoActions(SetMemory(RC + 4, SetTo, 64 * 65537))
        for b, sl in zip(bitXY(), l):
            IF, condition, _ = sl
            IF()(condition)
    ...
    if EUDIf()([SelCount >= 1, QGCActivated >= 1]):
        QueueGameCommand_NSelect(SelCount, SelMem._epd)
    EUDEndIf()
